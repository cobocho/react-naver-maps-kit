...
`Kmz`는 KMZ(Keyhole Markup Language Zipped) 파일을 URL로 불러와 지도에 렌더링하는 컴포넌트입니다.
KMZ는 KML 파일이 ZIP으로 압축된 형식으로, 내부에서 자동으로 압축을 해제한 후 `naver.maps.Data.addKml`로 Feature를 추가합니다.
...
| `onDataError`     | `(error: Error) => void`                   | fetch, 압축 해제 또는 KML 파싱 실패 시 호출   |
| `onFeaturesAdded` | `(features: naver.maps.Feature[]) => void` | KML에서 Feature 추가 완료 시 호출             |
...
- `url` prop이 변경되면 기존 Feature를 제거하고 새 KMZ 파일을 fetch하여 로드합니다.
- KMZ 파일을 fetch한 후 ZIP 압축을 해제하고, 내부의 `.kml` 파일을 찾아 파싱합니다.
- 비동기 fetch 중 컴포넌트가 언마운트되면 요청을 자동으로 abort합니다.
- ZIP 해제 또는 KML 파싱에 실패하면 `onDataError`를 호출합니다.
