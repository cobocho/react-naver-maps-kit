...
import { RECTANGLE_BOUNDS_1, RECTANGLE_BOUNDS_2, RECTANGLE_BOUNDS_3 } from "./app/constants";
...
test.describe("1. 렌더링/생성/정리 (스모크)", () => {
  test.beforeEach(async ({ page }) => {
    await page.goto("/#/rectangle/smoke");
  });

  test("R-01: 기본 Rectangle 렌더링 + onRectangleReady 호출", async ({ page }) => {
    await expect(page.getByTestId("map-container")).toBeVisible({ timeout: MAP_LOAD_TIMEOUT });

    await expect
      .poll(async () => Number(await page.getByTestId("rectangle-ready-count").textContent()))
      .toBeGreaterThanOrEqual(1);
  });

  test("R-02: 언마운트 시 onRectangleDestroy 호출 + 치명 에러 없음", async ({ page }) => {
    await expect(page.getByTestId("map-container")).toBeVisible({ timeout: MAP_LOAD_TIMEOUT });
    await expect
      .poll(async () => Number(await page.getByTestId("rectangle-ready-count").textContent()))
      .toBeGreaterThanOrEqual(1);
    ...
    await page.getByTestId("toggle-rectangle").click();
    await expect(page.getByTestId("show-rectangle")).toHaveText("false");
    await expect(page.getByTestId("rectangle-destroyed")).toHaveText("true");
  ...
  test("R-03: map prop 미지정 시 context map에 바인딩된다", async ({ page }) => {
    await expect(page.getByTestId("map-container")).toBeVisible({ timeout: MAP_LOAD_TIMEOUT });
    await expect
      .poll(async () => Number(await page.getByTestId("rectangle-ready-count").textContent()))
      .toBeGreaterThanOrEqual(1);
  ...
  test("R-04: 재마운트 시 onRectangleReady 카운트가 증가한다", async ({ page }) => {
    await expect(page.getByTestId("map-container")).toBeVisible({ timeout: MAP_LOAD_TIMEOUT });
    const before = Number(await page.getByTestId("rectangle-ready-count").textContent());
...
test.describe("2. bounds/options 반영", () => {
  test.beforeEach(async ({ page }) => {
    await page.goto("/#/rectangle/bounds-options");
    await expect(page.getByTestId("map-ready")).toHaveText("true", { timeout: MAP_LOAD_TIMEOUT });
  });

  test("R-05: bounds 변경이 getBounds에 반영된다", async ({ page }) => {
    ...
    expectBoundsClose(bounds, RECTANGLE_BOUNDS_2);
  });

  test("R-06: 빠른 연속 bounds 변경 시 최종 bounds로 수렴한다", async ({ page }) => {
    ...
    await expect
      .poll(async () => {
        ...
        return (
          Math.abs(bounds.south - RECTANGLE_BOUNDS_3.south) < 0.01 &&
          Math.abs(bounds.west - RECTANGLE_BOUNDS_3.west) < 0.01 &&
          Math.abs(bounds.north - RECTANGLE_BOUNDS_3.north) < 0.01 &&
          Math.abs(bounds.east - RECTANGLE_BOUNDS_3.east) < 0.01
        );
      })
      .toBe(true);
  });

  test("R-07: visible 토글이 반영된다", async ({ page }) => {
  ...
  test("R-08: clickable 토글이 반영된다", async ({ page }) => {
  ...
  test("R-09: zIndex 변경이 반영된다", async ({ page }) => {
  ...
  test("R-10: fillColor/fillOpacity 변경이 반영된다", async ({ page }) => {
  ...
  test("R-11: strokeColor/strokeOpacity/strokeWeight 변경이 반영된다", async ({ page }) => {
  ...
  test("R-12: strokeStyle/strokeLineCap/strokeLineJoin 변경이 반영된다", async ({ page }) => {
  ...
  test("R-13: bounds/clickable/visible/zIndex changed 이벤트가 발생한다", async ({ page }) => {
  ...
  test("R-14: fill/stroke changed 이벤트가 발생한다", async ({ page }) => {
  ...
  test("R-15: strokeLineCap/strokeLineJoin/strokeStyle changed 이벤트가 발생한다", async ({
    page
  }) => {
  ...
  test("R-16: getAreaSize는 양수를 반환한다", async ({ page }) => {
    await page.getByTestId("read-state").click();
...
test.describe("3. 포인터 이벤트", () => {
  test.beforeEach(async ({ page }) => {
    await page.goto("/#/rectangle/events");
    await expect(page.getByTestId("map-ready")).toHaveText("true", { timeout: MAP_LOAD_TIMEOUT });
    await page.getByTestId("clear-log").click();
  });

  test("R-17: onClick 이벤트가 호출된다", async ({ page }) => {
  ...
  test("R-18: onDblClick/onRightClick 이벤트가 호출된다", async ({ page }) => {
  ...
  test("R-19: onMouseOver/onMouseOut 이벤트가 호출된다", async ({ page }) => {
  ...
  test("R-20: onMouseDown/onMouseMove/onMouseUp 이벤트가 호출된다", async ({ page }) => {
  ...
  test("R-21: onTouchStart/onTouchMove/onTouchEnd 이벤트가 호출된다", async ({ page }) => {
  ...
  test("R-22: onMapChanged 이벤트가 호출된다", async ({ page }) => {
...
test.describe("4. Ref 기반 imperative 동작", () => {
  test.beforeEach(async ({ page }) => {
    await page.goto("/#/rectangle/ref");
    await expect(page.getByTestId("map-ready")).toHaveText("true", { timeout: MAP_LOAD_TIMEOUT });
  });

  test("R-23: ref.setBounds()로 bounds를 변경할 수 있다", async ({ page }) => {
  ...
  test("R-24: ref.setClickable()로 clickable을 변경할 수 있다", async ({ page }) => {
  ...
  test("R-25: ref.setVisible()로 visible을 변경할 수 있다", async ({ page }) => {
  ...
  test("R-26: ref.setZIndex()로 zIndex를 변경할 수 있다", async ({ page }) => {
  ...
  test("R-27: ref.setOptions()로 옵션을 부분 업데이트할 수 있다", async ({ page }) => {
  ...
  test("R-28: ref.setStyles()로 style을 업데이트할 수 있다", async ({ page }) => {
  ...
  test("R-29: ref.setMap(null/map) 호출 후 map 바인딩이 유지된다", async ({ page }) => {
  ...
  test("R-30: ref.getAreaSize/getDrawingRect/getElement/getBounds가 유효값을 반환한다", async ({
    page
  }) => {
    await page.getByTestId("ref-read-state").click();
    ...
    const bounds = parseJson<BoundsLiteral>(await page.getByTestId("ref-bounds").textContent());
    expectBoundsClose(bounds, RECTANGLE_BOUNDS_1);
  });

  test("R-31: ref.getOptions/getPanes/getProjection이 유효값을 반환한다", async ({ page }) => {
    await page.getByTestId("ref-read-state").click();
  ...
  test("R-32: map_changed 이벤트 카운트가 증가한다", async ({ page }) => {
...
/* ─── 5. 복수 Rectangle / 동적 추가·제거 ─── */

test.describe("5. 복수 Rectangle / 동적 추가·제거", () => {
  test.beforeEach(async ({ page }) => {
    await page.goto("/#/rectangle/multiple");
    await expect(page.getByTestId("map-ready")).toHaveText("true", { timeout: MAP_LOAD_TIMEOUT });
  });

  test("R-33: Rectangle 3개가 기본 렌더링된다", async ({ page }) => {
    await expect(page.getByTestId("rectangle-count")).toHaveText("3");
  ...
  test("R-34: Rectangle 동적 추가 시 카운트가 증가한다", async ({ page }) => {
    const before = Number(await page.getByTestId("rectangle-ready-count").textContent());

    await page.getByTestId("add-rectangle").click();
    await expect(page.getByTestId("rectangle-count")).toHaveText("4");
  ...
  test("R-35: Rectangle 동적 제거 시 destroy 콜백이 발생한다", async ({ page }) => {
    await page.getByTestId("remove-last-rectangle").click();
    await expect(page.getByTestId("rectangle-count")).toHaveText("2");

    await expect
      .poll(async () => Number(await page.getByTestId("rectangle-destroy-count").textContent()))
      .toBeGreaterThanOrEqual(1);
  });

  test("R-36: 첫 Rectangle visible 토글이 반영된다", async ({ page }) => {
  ...
  test("R-37: 둘째 Rectangle zIndex 변경이 반영된다", async ({ page }) => {
