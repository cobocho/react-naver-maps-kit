...
async function readText(page: Page, testId: string) {
...
test.describe("1. basic", () => {
  ...
  test("KMZ-02: style 변경 시 setStyle 반영 결과를 getStyle로 읽을 수 있다", async ({ page }) => {
...
test.describe("2. url switch", () => {
  test.beforeEach(async ({ page }) => {
    await page.goto("/#/kmz/url-switch");
    await waitForMapReady(page);
  });

  test("KMZ-03: 초기 URL(a) 로드 후 feature count를 읽을 수 있다", async ({ page }) => {
    await page.getByTestId("read-state").click();

    await expect.poll(async () => readNumber(page, "feature-count")).toBeGreaterThan(0);
    await expect(page.getByTestId("url-state")).toHaveText("/seoul.kmz");
  });

  test("KMZ-04: URL을 b로 변경하면 features가 재로드되고 count를 유지한다", async ({ page }) => {
    await page.getByTestId("read-state").click();
  ...
  test("KMZ-05: URL 전환 과정에서 정상 케이스는 error가 증가하지 않는다", async ({ page }) => {
    await page.getByTestId("set-url-b").click();
...
test.describe("3. events", () => {
  ...
  test("KMZ-06: pointer 이벤트 핸들러들이 Data 이벤트와 연결된다", async ({ page }) => {
  ...
  test("KMZ-07: add/remove/property_changed 이벤트 핸들러가 호출된다", async ({ page }) => {
    await page.getByTestId("clear-log").click();
...
test.describe("4. ref", () => {
  test.beforeEach(async ({ page }) => {
    await page.goto("/#/kmz/ref");
    await waitForMapReady(page);
    await page.getByTestId("ref-read-state").click();
  });

  test("KMZ-08: ref로 map binding/getAllFeature/toGeoJson을 조회할 수 있다", async ({ page }) => {
    await expect(page.getByTestId("map-bound")).toHaveText("true");
    await expect.poll(async () => readNumber(page, "feature-count")).toBeGreaterThan(0);
    await expect(page.getByTestId("geojson-type")).toHaveText("FeatureCollection");
  });

  test("KMZ-09: ref.setStyle 후 getStyle 결과가 갱신된다", async ({ page }) => {
  ...
  test("KMZ-10: ref.removeFeature 호출 시 feature count가 감소한다", async ({ page }) => {
...
test.describe("5. lifecycle", () => {
  test.beforeEach(async ({ page }) => {
    await page.goto("/#/kmz/lifecycle");
    await waitForMapReady(page);
  });

  test("KMZ-12: 마운트 시 onDataReady, 언마운트 시 onDataDestroy가 호출된다", async ({ page }) => {
    await expect.poll(async () => readNumber(page, "data-ready-count")).toBeGreaterThanOrEqual(1);

    await page.getByTestId("toggle-kmz").click();

    await expect(page.getByTestId("show-kmz")).toHaveText("false");
  ...
  test("KMZ-13: 다시 마운트하면 onDataReady 카운트가 증가한다", async ({ page }) => {
...
test.describe("6. error", () => {
  test.beforeEach(async ({ page }) => {
    await page.goto("/#/kmz/error");
    await waitForMapReady(page);
  });

  test("KMZ-14: 네트워크 실패 URL 로드 시 onDataError가 호출된다", async ({ page }) => {
  ...
  test("KMZ-15: 잘못된 KMZ 로드 시 onDataError가 호출된다", async ({ page }) => {
    const beforeErrorCount = await readNumber(page, "error-count");
  ...
  test("KMZ-16: 에러 이후 valid URL(b)로 복귀하면 다시 features가 추가된다", async ({ page }) => {
    await page.getByTestId("set-url-invalid-kmz").click();
