...
function normalizePaths(paths: unknown): PolygonPathsLiteral {
  ...
  return collection.map((path) => normalizePath(path)).filter((path) => path.length > 0);
}
...
function RefPage() {
  ...
  const readState = useCallback(() => {
    ...
    setRefBounds(
      toBoundsText(polygonRef.current?.getBounds() as naver.maps.LatLngBounds | undefined)
    );
      ...
      buttons={
        <>
          <button
            ...
            onClick={() =>
              polygonRef.current?.setPath(POLYGON_PATHS_2[0] as naver.maps.ArrayOfCoordsLiteral)
            }
          >
            setPath 2
          </button>
          <button
            ...
            onClick={() =>
              polygonRef.current?.setPaths(POLYGON_PATHS_3 as naver.maps.ArrayOfCoordsLiteral[])
            }
          >
            setPaths 3
          </button>
          <button
            ...
            onClick={() => polygonRef.current?.setClickable(true)}
          >
            clickable true
          </button>
          <button
            data-testid="ref-set-clickable-false"
            onClick={() => polygonRef.current?.setClickable(false)}
          >
            clickable false
          </button>
          <button
            data-testid="ref-set-visible-false"
            onClick={() => polygonRef.current?.setVisible(false)}
          >
            visible false
          </button>
          <button
            data-testid="ref-set-visible-true"
            onClick={() => polygonRef.current?.setVisible(true)}
          >
            visible true
          </button>
          <button
            data-testid="ref-set-zindex-555"
            onClick={() => polygonRef.current?.setZIndex(555)}
          >
            zIndex 555
          </button>
          <button
...
function MultiplePage() {
  ...
  return (
    <ScenarioLayout
      buttons={
        <>
          <button data-testid="add-polygon" onClick={addPolygon}>
            polygon 추가
          </button>
          <button data-testid="remove-last-polygon" onClick={removeLast}>
            마지막 제거
          </button>
          <button data-testid="toggle-first-visible" onClick={toggleFirstVisible}>
            첫 polygon visible 토글
          </button>
