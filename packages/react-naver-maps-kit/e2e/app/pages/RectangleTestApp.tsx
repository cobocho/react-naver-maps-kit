...
function randomBounds(seed: number): RectangleBoundsLiteral {
...
function SmokePage() {
  ...
  const rectangleRef = useRef<RectangleRef>(null);

  const [mapReady, setMapReady] = useState(false);
  const [rectangleReadyCount, setRectangleReadyCount] = useState(0);
  const [rectangleDestroyed, setRectangleDestroyed] = useState(false);
  ...
  const readBinding = useCallback(() => {
    const rectangleMap = rectangleRef.current?.getMap();
    const mapInstance = mapRef.current?.getInstance();

    setMapBound(String(Boolean(rectangleMap)));
    setMapEquals(String(Boolean(rectangleMap && mapInstance && rectangleMap === mapInstance)));
  }, []);
  ...
  return (
    <ScenarioLayout
      buttons={
        <>
          <button data-testid="toggle-rectangle" onClick={() => setShowRectangle((v) => !v)}>
            rectangle 토글
          </button>
      ...
      logs={
        <>
          ...
          <span data-testid="rectangle-ready-count">{rectangleReadyCount}</span>
          <span data-testid="rectangle-destroyed">{String(rectangleDestroyed)}</span>
          <span data-testid="show-rectangle">{String(showRectangle)}</span>
          ...
          >
            {showRectangle && (
              <Rectangle
                ...
                clickable={true}
                visible={true}
                onRectangleReady={() => setRectangleReadyCount((c) => c + 1)}
                onRectangleDestroy={() => setRectangleDestroyed(true)}
              />
            )}
          </NaverMap>
        </NaverMapProvider>
...
function BoundsOptionsPage() {
  ...
  const [mapReady, setMapReady] = useState(false);
  const [bounds, setBounds] = useState<RectangleBoundsLiteral>(RECTANGLE_BOUNDS_1);
  ...
  const readState = useCallback(() => {
    const styles = (rectangleRef.current?.getStyles() ?? {}) as Record<string, unknown>;
    const strokeStyleValue = rectangleRef.current?.getOptions("strokeStyle");
    const strokeLineCapValue = rectangleRef.current?.getOptions("strokeLineCap");
    const strokeLineJoinValue = rectangleRef.current?.getOptions("strokeLineJoin");

    setOptBounds(toBoundsText(rectangleRef.current?.getBounds() as naver.maps.LatLngBounds | undefined));
    setOptVisible(String(rectangleRef.current?.getVisible()));
    setOptClickable(String(rectangleRef.current?.getClickable()));
    setOptZIndex(String(rectangleRef.current?.getZIndex()));
    setOptAreaSize(String(rectangleRef.current?.getAreaSize() ?? ""));
  ...
  const rapidBounds = useCallback(() => {
    setBounds(RECTANGLE_BOUNDS_1);
    setTimeout(() => setBounds(RECTANGLE_BOUNDS_2), 50);
    setTimeout(() => setBounds(RECTANGLE_BOUNDS_3), 100);
  }, []);

  const triggerChanged = useCallback((eventName: string, payload: unknown) => {
    const rectangle = rectangleRef.current?.getInstance();

    if (!rectangle) {
      return;
    }

    naver.maps.Event.trigger(rectangle, eventName, payload);
  }, []);

  useEffect(() => {
    const rectangle = rectangleRef.current?.getInstance();
    if (!rectangle) return;
    naver.maps.Event.trigger(rectangle, "bounds_changed", rectangle.getBounds());
  }, [bounds]);
  ...
  return (
    <ScenarioLayout
      buttons={
        <>
          <button data-testid="set-bounds-2" onClick={() => setBounds(RECTANGLE_BOUNDS_2)}>
            bounds 2
          </button>
          <button data-testid="set-bounds-3" onClick={() => setBounds(RECTANGLE_BOUNDS_3)}>
            bounds 3
          </button>
...
function EventsPage() {
  ...
  const triggerEvent = useCallback((eventName: string) => {
    ...
    if (eventName === "map_changed") {
      naver.maps.Event.trigger(rectangle, eventName, rectangle.getMap());
      return;
    }

    naver.maps.Event.trigger(rectangle, eventName, {});
  }, []);
...
function RefPage() {
  ...
  const readState = useCallback(() => {
    const styles = (rectangleRef.current?.getStyles() ?? {}) as Record<string, unknown>;
    const drawingRect = rectangleRef.current?.getDrawingRect();

    setRefBounds(toBoundsText(rectangleRef.current?.getBounds() as naver.maps.LatLngBounds | undefined));
    setRefClickable(String(rectangleRef.current?.getClickable()));
    setRefVisible(String(rectangleRef.current?.getVisible()));
    setRefZIndex(String(rectangleRef.current?.getZIndex()));
    setRefAreaSize(String(rectangleRef.current?.getAreaSize() ?? ""));
    setRefMapBound(String(Boolean(rectangleRef.current?.getMap())));
    setRefElementExists(String(Boolean(rectangleRef.current?.getElement())));
    setRefDrawingRectExists(String(Boolean(drawingRect)));
    setRefOptionsExists(String(Boolean(rectangleRef.current?.getOptions())));
    setRefPanesExists(String(Boolean(rectangleRef.current?.getPanes())));
    setRefProjectionExists(String(Boolean(rectangleRef.current?.getProjection())));
  ...
  const triggerMapChanged = useCallback(() => {
    ...
    naver.maps.Event.trigger(rectangle, "map_changed", rectangle.getMap());
  }, []);
  ...
  return (
    <ScenarioLayout
      buttons={
        <>
          <button data-testid="ref-set-bounds-2" onClick={() => rectangleRef.current?.setBounds(RECTANGLE_BOUNDS_2)}>
            setBounds 2
          </button>
          <button data-testid="ref-set-clickable-true" onClick={() => rectangleRef.current?.setClickable(true)}>
            clickable true
          </button>
          <button data-testid="ref-set-clickable-false" onClick={() => rectangleRef.current?.setClickable(false)}>
            clickable false
          </button>
          <button data-testid="ref-set-visible-false" onClick={() => rectangleRef.current?.setVisible(false)}>
            visible false
          </button>
          <button data-testid="ref-set-visible-true" onClick={() => rectangleRef.current?.setVisible(true)}>
            visible true
          </button>
          <button data-testid="ref-set-zindex-555" onClick={() => rectangleRef.current?.setZIndex(555)}>
            zIndex 555
          </button>
          <button
            data-testid="ref-set-options"
            onClick={() => {
              const optionSetter = rectangleRef.current as RectangleRef & {
          ...
          <button
            ...
            onClick={() => {
              const styleSetter = rectangleRef.current as RectangleRef & {
...
function MultiplePage() {
  const [mapReady, setMapReady] = useState(false);
  const [rectangleReadyCount, setRectangleReadyCount] = useState(0);
  const [rectangleDestroyCount, setRectangleDestroyCount] = useState(0);
  const [items, setItems] = useState<RectangleItem[]>([
    { id: 1, bounds: RECTANGLE_BOUNDS_1, visible: true, zIndex: 1 },
    { id: 2, bounds: RECTANGLE_BOUNDS_2, visible: true, zIndex: 2 },
    { id: 3, bounds: RECTANGLE_BOUNDS_3, visible: true, zIndex: 3 }
  ]);
  ...
  return (
    <ScenarioLayout
      buttons={
        <>
          <button data-testid="add-rectangle" onClick={addRectangle}>rectangle 추가</button>
          <button data-testid="remove-last-rectangle" onClick={removeLast}>마지막 제거</button>
          <button data-testid="toggle-first-visible" onClick={toggleFirstVisible}>첫 rectangle visible 토글</button>
      ...
      logs={
        <>
          <span data-testid="map-ready">{String(mapReady)}</span>
          <span data-testid="rectangle-count">{items.length}</span>
          <span data-testid="rectangle-ready-count">{rectangleReadyCount}</span>
          <span data-testid="rectangle-destroy-count">{rectangleDestroyCount}</span>
          ...
          >
            {items.map((item) => (
              <Rectangle
                ...
                strokeWeight={2}
                onRectangleReady={() => setRectangleReadyCount((c) => c + 1)}
                onRectangleDestroy={() => setRectangleDestroyCount((c) => c + 1)}
              />
            ))}
          </NaverMap>
        </NaverMapProvider>
...
export const rectangleRoutes: Record<string, React.FC> = {
  "/rectangle/smoke": SmokePage,
  "/rectangle/bounds-options": BoundsOptionsPage,
  "/rectangle/events": EventsPage,
  "/rectangle/ref": RefPage,
  "/rectangle/multiple": MultiplePage
};
